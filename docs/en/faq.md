...
### 16. What datasets can I search for in the map search?
Geodisy provides map search functionality to supplement the FRDR discovery service. The map search indexes a subset of the datasets available through FRDR’s main search. The map search includes datasets with valid bounding box metadata from all repositories harvested by FRDR. Repository datasets harvested by FRDR that have location metadata and/or geospatial files are also included if the location metadata meets map search’s criteria for inclusion and/or the geospatial file can be interpreted by map search’s software. FRDR is actively working toward expanding the full location metadata and geospatial file interpretation capabilities to all other indexed repositories. More FAQ information about map search is available [here](https://github.com/ubc-library/geodisy/blob/master/Documentation/userguides/FAQ.md).
